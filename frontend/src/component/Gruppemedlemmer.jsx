...
export default function Gruppemedlemmer(){
    ...
    useEffect(() => {
      ...
      async function fetchAllMedlemmer() {
        const allMedlemmer = await client.fetch("*[_type == 'gruppemedlemmer']{_id, navn, slug, epost, studie, bilde, 'imageURL': bilde.asset->url}")
    ...
    <section id="member-list">
      ...
      {sanityMedlem?.map((m) => (<Link key={m._id} to={"/medlem/" + m.slug.current}> <Medlemskort key={m._id} m={m}/></Link>))}
    </section>)
